...
//            System.out.println("ApplicationContextInfoTest.findAllBean");   // soutm
    ...
    @Test
    @DisplayName("애플리케이션 빈 출력하기 (직접 등록한 애플리케이션 빈)")
    void findApplicationBeanApplication() {
        String[] beanDefinitionNames = ac.getBeanDefinitionNames();
    ...
    @Test
    @DisplayName("애플리케이션 빈 출력하기 (스프링이 내부에서 사용하는 빈)")
    void findApplicationBeanInfrastructure() {
        String[] beanDefinitionNames = ac.getBeanDefinitionNames();
        for (String beanDefinitionName : beanDefinitionNames) {
            BeanDefinition beanDefinition = ac.getBeanDefinition(beanDefinitionName);
